...
c.innerHTML=reports.length?reports.map(r=>` <div class="report-item" onclick="viewReport('${r.id}')"> <div class="rep-head"><div class="rep-title">📋 تقرير ${r.type==='auto'?'تلقائي':'يدوي'} — ${fmtD(r.date)}</div><div class="rep-date">${r.time}</div></div> <span class="rep-type ${r.type==='auto'?'rep-daily':'rep-manual'}">${r.type==='auto'?'تلقائي':'يدوي'}</span> <div style="font-size:11px;color:var(--t3);margin-top:6px;overflow:hidden;max-height:40px">${r.text.substring(0,100)}...</div> </div>`).join(’’):`<div class="empty"><div class="ei">📋</div><p>لا توجد تقارير<br><small>تُرسل تلقائياً كل 4 صباحاً</small></p></div>`;
}
...
document.getElementById(‘detailIG’).innerHTML=` <div class="ii"><div class="lb">الاسم</div><div class="vl">${emp.name}</div></div> <div class="ii"><div class="lb">اليوزر</div><div class="vl cyan">${emp.u}</div></div> <div class="ii"><div class="lb">الشفت</div><div class="vl">${shiftLabel(emp)}</div></div> <div class="ii"><div class="lb">الراتب/شهر</div><div class="vl gold">${fmtN(emp.sal)} د.ع</div></div> <div class="ii"><div class="lb">معدل اليوم</div><div class="vl gold">${fmtN(Math.round(emp.sal/30))} د.ع</div></div> <div class="ii"><div class="lb">نوع الدوام</div><div class="vl">${sh.full?'كامل ⭐':'نصفي'}</div></div> <div class="ii"><div class="lb">التقييم</div><div class="vl gold">${emp.rating||0} ⭐</div></div>`;
...
document.getElementById(‘dAttList’).innerHTML=att.length?att.map(a=>` <div class="hi"> <div class="hic ${a.type==='ci'?'in':'out'}">${a.type==='ci'?'✅':'🚪'}</div> <div class="hinfo"><div class="htype">${a.type==='ci'?'حضور':'انصراف'}${a.type==='co'&&a.durMins?` — ${Math.floor(a.durMins/60)}س ${a.durMins%60}د`:''}</div><div class="hdate">${fmtD(a.date)}</div></div> <div class="htime">${a.time}</div> </div>`).join(’’):`<div class="empty"><div class="ei">📭</div><p>لا توجد سجلات</p></div>`;
...
return `<div class="bi"><div><div class="br2">${b.note||b.type}</div><div class="bd">${fmtD(b.date)}</div></div> <div class="flex g8" style="align-items:center"><div class="ba">+${fmtN(b.amount)} د.ع</div> <button class="btn btn-dn btn-sm btn-ic" onclick="rmBon(${i})" title="إلغاء الحافز">🗑️</button></div></div>`;
...
return `<div class="di"><div><div class="br2">${d.reason}</div><div class="dd">${fmtD(d.date)}</div></div> <div class="flex g8" style="align-items:center"><div class="da">-${fmtN(d.amount)} د.ع</div> <button class="btn btn-dn btn-sm btn-ic" onclick="rmDed(${i})">🗑️</button></div></div>`;
...
// Export to window
window.buildReportText = buildReportText;
window.saveReportToArchive = saveReportToArchive;
window.genAndSaveReport = genAndSaveReport;
window.sendTgReport = sendTgReport;
window.renderReportsList = renderReportsList;
window.viewReport = viewReport;
window.sendSavedReportToTg = sendSavedReportToTg;
window.handleShiftChange = handleShiftChange;
window.openAddEmp = openAddEmp;
window.addEmp = addEmp;
window.openDetail = openDetail;
window.openDetailBon = openDetailBon;
window.renderDBon = renderDBon;
window.renderDDed = renderDDed;
window.addBonD = addBonD;
window.rmBon = rmBon;
window.addDedD = addDedD;
window.rmDed = rmDed;
window.saveSal = saveSal;
window.saveShift = saveShift;
window.saveEmpPw = saveEmpPw;
window.saveLv = saveLv;
window.saveNotes = saveNotes;
window.deleteEmp = deleteEmp;
window.qDel = qDel;
window.showDTab = showDTab;
window.qInc = qInc;
window.addCustomBon = addCustomBon;
window.openQS = openQS;
window.saveQS = saveQS;
window.changeAdminCreds = changeAdminCreds;
window.saveWorkLocation = saveWorkLocation;
window.useCurrentAsWork = useCurrentAsWork;
window.saveTgId = saveTgId;
window.testTg = testTg;
