...
function doLogin(){
  ...
  if(u===creds.u&&p===creds.pw){
    ...
    const goAdmin=()=>{showScreen('adminScreen'); renderAdmin(); startTimers(); showToast('مرحباً يا سجاد 👑','s');};
  ...
  // فحص الموظفين — جرب Firebase أولاً لو متصل
  const tryEmpLogin=()=>{
    ...
    if(emp){
      ...
      const goEmp=(freshEmp)=>{showScreen('empScreen'); loadEmpScreen(freshEmp||emp); startGPS(); const h=new Date().getHours(); showToast(`${h<12?'صباح الخير':h<17?'مساء الخير':'ليلة سعيدة'} ${(freshEmp||emp).name} 😊`,'i');};
      if(fbDB&&fbSyncEnabled){
        fbDB.ref('ccs').once('value').then(snap=>{
          const cloud=snap.val()||{};
          SYNC_KEYS.forEach(k=>{if(cloud[k]!==null&&cloud[k]!==undefined)localStorage.setItem('ccs2_'+k,JSON.stringify(cloud[k]));});
