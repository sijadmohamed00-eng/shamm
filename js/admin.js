...
return `<tr> <td><div class="tname"><div class="eav">${e.name[0]}</div>${e.name}</div></td> <td><span class="shbadge">${shiftLabel(e)}</span></td> <td>${ciTime}</td><td>${coTime}</td><td>${dur}</td><td>${tag}</td> </tr>`;
}).join(’’);
}
...
return `<tr><td>${i+1}</td> <td><div class="tname"><div class="eav">${e.name[0]}</div><div><div>${e.name}</div><div class="muted">${e.u}</div></div></div></td> <td><code style="background:var(--bg2);padding:2px 6px;border-radius:4px;font-size:11px">${e.u}</code></td> <td><span class="shbadge">${shiftLabel(e)}</span></td> <td><span class="gnum">${fmtN(e.sal)}</span></td> <td>${tag}</td> <td><div class="flex g8"> <button class="btn btn-sc btn-sm" onclick="openDetail('${e.id}')">📋 تفاصيل</button> <button class="btn btn-ok btn-sm" onclick="openDetailBon('${e.id}')">🎁</button> <button class="btn btn-dn btn-sm btn-ic" onclick="qDel('${e.id}')">🗑️</button> </div></td></tr>`;
}).join(’’);
}
...
return `<tr> <td><div class="tname"><div class="eav">${e.name[0]}</div>${e.name}</div></td> <td><span class="gnum">${fmtN(e.sal)}</span></td> <td><span class="tgrn bold">${fmtN(c.earnedSalary)}</span></td> <td><span style="color:var(--purple)">+${fmtN(c.shiftBonTotal)}</span></td> <td><span class="tgrn">+${fmtN(c.salesBonTotal+c.otherBonTotal)}</span></td> <td><span class="tred">-${fmtN(c.totalDed)}</span></td> <td><span class="gnum" style="font-size:14px">${fmtN(c.net)}</span></td> <td><button class="btn btn-sc btn-sm" onclick="openDetail('${e.id}');setTimeout(()=>showDTab('sal'),100)">📊</button></td> </tr>`;
}).join(’’);
}
...
return `<tr> <td><div class="tname"><div class="eav">${e.name[0]}</div>${e.name}</div></td> <td><span class="shbadge">${shiftLabel(e)}</span></td> <td>${sh.full?'<span class="tgld bold">كامل</span>':'<span class="tcyn">نصفي</span>'}</td> <td>${sh.full?'<span class="tgrn bold">10,000 د.ع</span>':'<span class="muted">—</span>'}</td> <td><button class="btn btn-sc btn-sm" onclick="openQS('${e.id}','${e.name}','${e.sh}')">✏️</button></td> </tr>`;
}).join(’’);
}
...
rb.innerHTML=all.slice(0,15).length?all.slice(0,15).map(b=>` <div class="bi"><div><div class="br2">${b.ename} — ${b.note||b.type}</div><div class="bd">${fmtD(b.date)}</div></div> <div class="flex g8" style="align-items:center"><div class="ba">+${fmtN(b.amount)} د.ع</div> <button class="btn btn-dn btn-sm btn-ic" onclick="removeBonFromList('${b.eid}',${b.idx})">🗑️</button> </div></div>`).join(’’):`<div class="empty" style="padding:20px"><div class="ei" style="font-size:26px">🎁</div><p>لا توجد حوافز</p></div>`;
}
...
return `<div class="arch-period" onclick="showAdminArchDetail('${per.key}')"> <div class="arch-ph"> <div> <div style="font-size:14px;font-weight:700">🗄️ ${per.label}</div> <div style="font-size:11px;color:var(--t2);margin-top:3px">يوم الصرف: ${fmtD(per.payDate)} | أُرشف: ${new Date(per.archivedAt).toLocaleDateString('ar-IQ')}</div> </div> <div style="text-align:left"> <div class="gnum" style="font-size:15px">${fmtN(totalNet)} د.ع</div> <div style="font-size:11px;color:var(--t2)">${empsToShow.length} موظف</div> </div> </div> </div>`;
}).filter(Boolean).join(’’);
...
document.getElementById(‘archDetailContent’).innerHTML=` <div style="background:rgba(240,192,64,.06);border:1px solid rgba(240,192,64,.2);border-radius:12px;padding:14px;margin-bottom:14px"> <div style="font-size:15px;font-weight:800;color:var(--gold);margin-bottom:4px">${per.label}</div> <div style="font-size:12px;color:var(--t2)">يوم الصرف: ${fmtD(per.payDate)} | إجمالي: ${fmtN(totalNet)} د.ع</div> </div> <div class="tw"><table><thead><tr><th>الموظف</th><th>حضور</th><th>مكتسب</th><th>حوافز</th><th>خصم</th><th>إجمالي</th></tr></thead><tbody> ${per.employees.map(e=>`<tr>
...
const emps=DB.get(‘emps’)||[];
// Full shift list
document.getElementById(‘promptFullShiftList’).innerHTML=emps.map(e=>` <div class="prompt-emp-row"> <div><div class="prompt-emp-name">${e.name}</div><div class="prompt-emp-status">${shiftLabel(e)}</div></div> <label style="display:flex;align-items:center;gap:6px;cursor:pointer"> <input type="checkbox" id="pfs_${e.id}" style="width:16px;height:16px;accent-color:var(--gold)"> <span style="font-size:12px;color:var(--t2)">شفت كامل</span> </label> </div>`).join(’’);
// Leave list
document.getElementById(‘promptLeaveList’).innerHTML=emps.map(e=>` <div class="prompt-emp-row"> <div class="prompt-emp-name">${e.name}</div> <label style="display:flex;align-items:center;gap:6px;cursor:pointer"> <input type="checkbox" id="plv_${e.id}" style="width:16px;height:16px;accent-color:var(--purple)"> <span style="font-size:12px;color:var(--t2)">إجازة</span> </label> </div>`).join(’’);
// Cheese bonus
document.getElementById(‘promptCheeseList’).innerHTML=emps.map(e=>` <div class="prompt-emp-row"> <div class="prompt-emp-name">${e.name}</div> <div style="display:flex;align-items:center;gap:6px"> <input type="number" class="prompt-cheese-input" id="pch_${e.id}" placeholder="0" min="0"> <span style="font-size:11px;color:var(--t2)">قطعة</span> </div> </div>`).join(’’);
...
window.renderQuickAtt = renderQuickAtt;
window.renderEmpTable = renderEmpTable;
window.renderSalTable = renderSalTable;
window.renderShTable = renderShTable;
window.renderIncTab = renderIncTab;
window.removeBonFromList = removeBonFromList;
window.renderRatingTab = renderRatingTab;
window.setRating = setRating;
window.renderMsgSelect = renderMsgSelect;
window.renderSentMessages = renderSentMessages;
window.deleteMessage = deleteMessage;
window.renderAdminArchive = renderAdminArchive;
window.showAdminArchDetail = showAdminArchDetail;
window.updateCharts = updateCharts;
window.showATab = showATab;
window.open3amPromptAuto = open3amPromptAuto;
window.open3amPromptManual = open3amPromptManual;
window.open3amPrompt = open3amPrompt;
